...
@section('content')
  ...
  <section class="content-header">
      <h1 class="text-center font-weight-600">
          @if(Request::is('admincp/posts/create'))
              Thêm bài viết
              <?php 
                  $url = url('admincp/posts/');
                  $method = 'POST';
              ?>
          @else
              Sửa bài viết
              <?php 
                  $url = url('admincp/posts/' . Request::route('post'));
                  $method = 'PUT';
              ?>
          @endif
              ...
              <div class="card">
                  <div class="card-body">
                      ...
                      <div class="form-group">
                          ...
                          <textarea class="form-control" id="content_ckeditor" name="content">{{ isset($data->content) ? $data->content : '' }}</textarea>
  ...
  <script>
        ...
        $('#save-change').click(function(){
            ...
            var data    = {
                ...
                content         : CKEDITOR.instances.content_ckeditor.getData(),
                ...
                success: function(response) {
                    if(response.status == 200){
                        Swal.fire({
                            ...
                            if (result.value) {
                                window.location.replace("{{ url('admincp/posts') }}");
                            }
                        });
                    }
                },
