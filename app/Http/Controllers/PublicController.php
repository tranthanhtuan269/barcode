...
class PublicController extends Controller
{
    	...
    	if( isset($_GET['barcode']) ){
	        try{
				...
				if ($data->barcode == '') {
					...
					return redirect()->route('seo-barcode', ['slug' => \Illuminate\Support\Str::of($data->name)->slug("-"), 'id' => $trimBarcode]);
				}
			...
			$mostViewArticles = \Cache::rememberForever('most_view_article_home', function () {
				return Article::select('title', 'slug', 'post_public_time', 'image', 'id','count_view','rate')->where('status', 1)->orderBy('count_view', 'desc')->take(12)->get();
			});
			$latestArticles = \Cache::rememberForever('latest_article_home', function () {
				return Article::select('title', 'slug', 'post_public_time', 'image', 'description', 'id','count_view','rate')->where('status', 1)->where('post_public_time', '<=', date('Y-m-d H:i:s'))->orderBy('post_public_time', 'desc')->take(6)->get();
			});
			$page = Page::where('slug','home')->first();
			if ($page) {
				return view('layouts_frontend.dashboard',compact('page', 'mostViewArticles', 'latestArticles'));
			}else {
				return view('layouts_frontend.dashboard', compact('articles', 'mostViewArticles', 'latestArticles'));
			}
		}
    }
    ...
    public function getSearchBarcode(Request $request, $slug){
		...
		if($slug2 == $slug){
			if($data && $data->show_status == 1){
				$data->view_count ++;
				$data->save();
				if($data->related_articles != null && $data->related_articles != "null" ) {
    ...
    public function getSlugAjax(Request $request){
        ...
        if( !isset($data) ){
            ...
            if( !empty($data->barcode) ){
            	...
            	if( !empty($data->avg_price) ){
            		$arr_avg_price = (explode(" ",$data->avg_price));
            		$avg_price = $arr_avg_price[0];
            		$currency_unit = isset($arr_avg_price[1]) ? $arr_avg_price[1] : "USD";
            		if ($currency_unit == "") {
            			$currency_unit = "USD";
            		}
            	}
		...
		if ($data->barcode == '') {
			...
			return response()->json(['status' => 200, 'slug' => \Illuminate\Support\Str::of($data->name)->slug("-")]);
		}
    }
}
